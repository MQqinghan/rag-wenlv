...
# BGE-M3 模型非线程安全：批量导入会为每个文件起独立线程并发调 encode，
# 并发下 FlagEmbedding 内部 tokenizer 状态互踩（报 'list' object has no attribute 'keys'），
# 因此所有 encode 调用必须串行化（GPU 推理本身也是串行吞吐最优）。
_model_lock = threading.Lock()


def get_bge_m3_ef() -> BGEM3EmbeddingFunction:
    """
    获取BGE-M3模型单例对象，自动加载环境变量配置（双重检查锁，防并发重复初始化）
...
def generate_embeddings(texts: list[str]) -> dict[str, list]:
    ...
    try:
        # 加载BGE-M3模型单例
        model = get_bge_m3_ef()
        # 模型编码生成向量：加锁串行化（模型非线程安全，批量导入多线程并发会崩）
        with _model_lock:
